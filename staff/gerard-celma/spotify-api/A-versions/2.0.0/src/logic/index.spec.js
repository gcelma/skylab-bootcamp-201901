...
const { DB_URL, SPOTIFY_API_TOKEN, SECRET } = process.env
...
describe('logic', () => {
    ...
    before(() =>
        MongoClient.connect('mongodb://localhost:27017/skylab', { useNewUrlParser: true })
            .then(_client => {
                client = _client
        ...
        it('should succeed on correct credentials', () => 
            logic.authenticateUser(email,password)
                .then(({ id, token }) => {
                    ...
                    expect(jwt.verify(token, 'tachimelody')).toBeTruthy()
                })
            )    
    })
    ...
    describe('retrieve user', () => {
        ...
        beforeEach(() =>
            users.register(name, surname, email, password)
                .then(() => users.authenticate(email, password))
                .then(({ id, token }) => {
                    _id = id
                    _token = token
                })
